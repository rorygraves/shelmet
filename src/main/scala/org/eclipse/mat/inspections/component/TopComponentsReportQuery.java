...
public class TopComponentsReportQuery implements IQuery {
    ...
    public IResult execute(IProgressListener listener) throws Exception {
        ...
        for (Record record : loaders) {
            ...
            SnapshotQuery query = SnapshotQuery.lookup("component_report", snapshot)
                    .setArgument("objects", record.objects);
            query.setArgument("aggressive", aggressive);
            IResult report = query.execute(listener);

            QuerySpec spec = new QuerySpec(MessageUtil.format("{0} ({1,number,percent})",
